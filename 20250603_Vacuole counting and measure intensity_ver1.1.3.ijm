 ...
 * ---2. Use actin channel to identify cell contour and filter with the area size.
 ...
 * ---4. Cell no was counted by the DAPI channel to avoid the double nuclei over counting.
 ...
 * 
 * -ver1.1.3(editted by 2025/6/3)
 * ---1. The nucleus number is countd by the DAPI channel.  
 * ---2. The cell area is measured from the actin channel.
...
//Prepare result table
    ...
    CellAreaArray = newArray(0);
    NucleusnoArray = newArray(0);
    ...
    Table.setColumn("Cell Area", CellAreaArray);//Actin channel
    Table.setColumn("Nucleus no.", NucleusnoArray);//DAPI channel
...
dir1 = getDirectory("Folder of input images");
...
//Draw cell outline by actin
    ...
    setAutoThreshold("Mean dark no-reset");
    run("Analyze Particles...", "size=30-Infinity summarize add");
    Actinno = roiManager("count");
    roiManager("Save", path + File.separator + Name+ "_Cell.zip");
    selectWindow("Summary");
    Cellarea = Table.get("Total Area", 0);
    a = Table.size;
    if (a>0) {
...
//Count nucleus no by DAPI stain
   	 selectImage("DAPI");
...
//Define location of Candida from smaller DAPI stain areas or labeled Candida
  ...
  //Generate Candida location by filtering out FungalGFP without fungal DNA
...
//Measure LC3 intensity around vacuoles
   ...
   if (CandidaNo>0) {
    ...
    //print(tifName + "      has   " + c + "   Vacuoles in   " + Nucleusno + "   cells with average LC3 intensity " + VacLC3IntensityAvg + " /vacuole.");
    FilenameArray = Array.concat(FilenameArray,tifName);
    CellAreaArray = Array.concat(CellAreaArray, Cellarea);
    NucleusnoArray = Array.concat(NucleusnoArray,Nucleusno);
...
//Generate result table
    ...
    Table.setColumn("File name", FilenameArray);
    Table.setColumn("Cell Area", CellAreaArray);
    Table.setColumn("Nucleus no.", NucleusnoArray);
