/*20250506_Vacuole counting and measure intensity ver 1.1.0
 * Dependency: CLiJ
 ...
 * ----4. Elevate the size criteria of fungal DNA.
 */
...
//Get holes from LC3 channel
  //Get LC3 Hole in the image
    selectWindow("LC3");
    run("Duplicate...", "title=LC3VacuoleIntensity");
    selectWindow("LC3");
    run("Enhance Contrast", "saturated=0.35");
    roiManager("Open", path + File.separator + Name+ "_Cell.zip");
    ...
    // Tubeness
     ...
     //Get LC3 Hole in cells
       ...
       run("Open");
       setAutoThreshold("Mean dark no-reset");
       roiManager("Open", path + File.separator + Name + "_nucleus.zip");
       selectWindow("Result of Lateralized LC3 Maxima");
       roiManager("Show All without labels"); 
       ...
       c = roiManager("count");
       if (c>0) {
       	roiManager("Save", path + File.separator + Name + "_LC3Hole.zip");
       	roiManager("reset");
       }
       if (c==0){
       	VacuoleNoArray == 0;
...
//Define location of Candida from smaller DAPI stain areas or labeled Candida
   //Generate Bac DNA mask   
     ...
     run("Watershed");
     //run("Threshold...");
     run("Analyze Particles...", "size=1.0-10.00 add");
     roiManager("Save", path + File.separator + Name + "_FungalDNA.zip");
     newImage("FungalDNA", "8-bit black", width, height, 1);
   ...
   //Generate Candida location from DAPI and Candida channel
     ...
     run("Convert to Mask");
     imageCalculator("Add create", "Candida","FungalDNA");
   ...
   //Document 
     selectImage("Candida");
     saveAs("tiff", path + File.separator + Name + "_CandidaMask.tif");
     close();
...
//Justify LC3 surrounded Candida
  roiManager("Open", path + File.separator + Name + "_LC3Hole.zip");
  roiManager("Measure");
  c =roiManager("count");
  nonVC = newArray(0);
  for (i = 0; i < c; i++) {
