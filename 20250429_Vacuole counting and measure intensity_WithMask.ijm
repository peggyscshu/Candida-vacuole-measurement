/*20250429_Vacuole counting and measure intensity
 ...
 * 
 * This edition provides intermediate image masks and ROIs for visualization and verification purposes.
 */
...
//Draw cell outline to capture holes nearby cell membrane
    ...
    roiManager("select", a);
    roiManager("delete");
    roiManager("Save", path + File.separator + tifName + "_Cell.zip");
  //Create cell mask
...
//Count cell no
     ...
     roiManager("Save", path + File.separator + tifName + "_nucleus.zip");
...
//Get holes from LC3 channel
  //Get LC3 Hole
    ...
    roiManager("Open", path + File.separator + tifName + "_Cell.zip");
    ...
    // Tubeness
     ...
     //Get LC3 Hole
       ...
       run("Open");
       setAutoThreshold("Mean dark no-reset");
       roiManager("Open", path + File.separator + tifName + "_nucleus.zip");
       selectWindow("Result of Lateralized LC3 Maxima");
       roiManager("Show All without labels"); 
       ...
       roiManager("Save", path + File.separator + tifName + "_LC3Hole.zip");
       c = roiManager("count");
       if (c>0) {
       	roiManager("reset");
       }
...
//Define location of Candida from smaller DAPI stain areas or labeled Candida
   //Generate Bac DNA mask   
     ...
     roiManager("Save", path + File.separator + tifName + "_bacDNA.zip");
   ...
   //Remove the cell nucleus from CandidaLocation mask
     ...
     setForegroundColor(0, 0, 0);
     roiManager("Fill");
     saveAs("tiff", path + File.separator + tifName + "_CandidaLocation.tif");
   //Document 
     selectImage("Candida");
     saveAs("tiff", path + File.separator + tifName + "_CandidaMask.tif");
     close();
     selectImage("BacDNA");
     saveAs("tiff", path + File.separator + tifName + "_BacIMask.tif");
     close();
     selectImage("DAPI");
     close();
     c = roiManager("count");
...
//Justify LC3 surrounded Candida
  roiManager("Open", path + File.separator + tifName + "_LC3Hole.zip");
  roiManager("Measure");
  c =roiManager("count");
  nonVC = newArray(0);
  for (i = 0; i < c; i++) {
    ...
    if (nonVC.length >0) {
         ...
         c =roiManager("count");
         if (c>0) {
         	roiManager("Save", path + File.separator + tifName + "_Vacuole.zip");
            ...
            selectWindow("Result of LC3VacuoleIntensity");
            saveAs("tiff", path + File.separator + tifName + "_Extracted LC3 around vacuoles.tif");
         }
         else{
         	VacLC3Intensity = 0;
